...
function forwardEvent(event, source, target) {
  ...
  target.on('newListener', function (name) {
    if (name === event && target.listeners(name).length === 0) {
      source.on(event, doForwardEvent);
    }
  });
  target.on('removeListener', function (name) {
    if (name === event && target.listeners(name).length === 1) {
      source.removeListener(event, doForwardEvent);
    }
  });
...
function Server(options) {
  ...
  if ((options.key && options.cert) || options.pfx) {
    ...
    forwardEvent('upgrade', this._server, this);
    forwardEvent('timeout', this._server, this);
  }
...
OutgoingRequest.prototype._fallback = function _fallback(request) {
  ...
  forwardEvent('error', this.request, this);
  forwardEvent('upgrade', this.request, this);
